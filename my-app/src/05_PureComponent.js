...
/*
 * 1. PureComponent ?
    class형 컴포넌트는 Component와 PureComponent 가 있음.

    두 함수 모두 props와 state의 변경에 따라 render함수를 호출하는데 변경에 대한 기준이 약간 다름

    Component에서는 setState함수 호출 시 내부적으로 변경하고자 하는 state의 값이 같더라도, 변경으로 간주하고
    render() 함수를 호출하였음.
...
class Pure extends PureComponent {
    ...
    stateChange = (flag) => {
        if(flag == 'string') {
            this.setState({StateString : 'react'}); // 렌더 함수 호출하지 않음
        }else if(flag == 'array'){
            this.setState({StateArray: ['react']}); // 렌더 함수 호출함
        }else if(flag == 'object'){
            this.setState({StateObject: {react : 'react'}}); // 랜더 함수 호출함
        }
    }
    /*
        객체의 경우 변수값이 아니라 객체의 주소값을 통해 비교를 수행하기 때문에
        기존 state에 담겨있던 StateObject나, StateArray는 서로 완전 다른 값이라고
        판단하고 render메서드 호출함
    */
    ...
    render(){
        ...
        return(
            <div style={{padding:"0px"}}>
                ...
                state ::: StateArray : {this.state.StateArray}
...
class Shallow extends Component{
    ...
    */
    ...
    stateChange = (flag) => {
        if(flag == 'string') {
            this.setState({StateString : 'react2'}); // 렌더 함수 호출하지 않음
        }else if(flag == 'array'){
            this.setState({StateArray: ['react2']}); // 렌더 함수 호출함
        }else if(flag == 'object'){
            this.setState({StateObject: {react : 'react2'}}); // 랜더 함수 호출함
        }
    }
