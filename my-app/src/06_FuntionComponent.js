/*
    ...
    부모 컴포넌트에게 props와 context를 파라미터로 전달받아 사용하고
    render함수가 없기 때문에 return만 사용하여 화면을 그려준다

    함수형 컴포넌트에서 state와 생명주기 함수의 기능을 사용해야하는경우 hook추가한다.
...
function FunctionComponent(props){
    ...
    */
    ...
    /*
        useEffect는 생명주기 함수 중 componentDidMount와 동일한 기능을 수행함
        return 되는 html들이 화면에 그려진 후 (랜더링 완료 후) 실행됨.
        최초 페이지 로딩될 때 한번 실행되고, setString()함수로 state값이 변경되면 다시 한 번 랜더링되면서 
        랜더링 완료 후 재실행된다.
    */
   return (
...
/*
    함수형 컴포넌트의 장점
     1. render함수가 없기 때문에 컴포넌트 마운트 속도가 class컴포넌트에 비해서 빠르다
     2. 가독성이 좋다
     3. this를 사용하지 않기 때문에 props의 값을 유지할 수가 있다.
      - class component에서는 this.props를 통해 props에 접근했기 때문에 props의 값이 변경될 위험이 있었다.
     4. 유일한 단점은 생명주기 함수나, state관리를 할 수 없다는 점인데, react Hook의 등장으로 단점이 사라져버림

     리액트에서는 hook의 등장 이후로 class component 방식보다 함수형 컴포넌트 방식으로 프로그래밍하는 것을 권장한다.
