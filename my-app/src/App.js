...
*/
import LifecycleUpdate from './01_ Lifecycle';
...
/*
  ...
  따라서 개발자는 복잡한 자바스크립트 코드를 짤 필요없이 동적으로 추가되는 DOM요소를 단순 코드 선언만으로 만들 수 있다.
  (선언적 방식)

  내가 작성한 JSX코드가 실제로 시랭될 때 리액트가 브라우저에서 구동되는 자바스크립트 코드로 자동으로 컴파일되면서
  구동이 된다.
*/
function App() {
  return (
    ...
    <div>
      {/*jsx 문법 영역 안에서 주석을 사용하는 방법*/}
      {/* <h2>start React</h2> */}
      {/* <h2>HELLO WORLD</h2> */}
      {/* import 해온 컴포넌트 클래스를 그대로 태그(마크업)으로서 사용하면 된다 */}
      {/* <LifecycleUpdate prop_value="From App.js"></LifecycleUpdate> */}
      {/* 
        props? 부모 컴포넌트가 자식컴포넌트에 넘겨주는 "객체" 형태의 데이터({key:value})
